...
function watchedWorkflowNames() {
  return new Set(
    String(process.env.KANAREK_WATCHED_WORKFLOWS ?? '')
      .split(',')
      .map((name) => name.trim())
      .filter(Boolean),
  );
...
async function pendingWatchedRuns(
  github,
  owner,
  repo,
  sha,
  ...
  if (process.env.KANAREK_COALESCE_WORKFLOW_RUNS === 'false') return [];
  ...
  try {
    ...
    return runs.filter(
      (run) =>
        run.id !== currentWorkflowRunId &&
        watched.has(run.name) &&
        run.status !== 'completed',
    );
  } catch (error) {
    core.warning(`Kanarek could not coalesce workflow runs: ${error.message}`);
    return [];
  }
}
...
function belongsToRun(check, runId) {
  ...
  return String(check.details_url ?? '').includes(`/actions/runs/${runId}`);
}

async function checks(github, owner, repo, sha, currentRunId) {
  ...
  const runs = allRuns.filter((item) => !belongsToRun(item, currentRunId));
  const statuses = (
...
async function processOne(github, context, owner, repo, number, core) {
  const pr = await pull(github, owner, repo, number);

  if (context.eventName === 'workflow_run') {
    const waiting = await pendingWatchedRuns(
      github,
      owner,
      repo,
      pr.head.sha,
    ...
    if (waiting.length) {
      core.info(
        `PR #${number}: waiting for ${waiting.map((run) => run.name).join(', ')}.`,
      );
      return;
    }
  }
  ...
  const [files, branch, ci, review, oldComments] = await Promise.all([
    ...
    checks(github, owner, repo, pr.head.sha, context.runId),
...
module.exports = async function run({ github, context, core }) {
  ...
  for (const number of pullNumbers) {
    try {
      await processOne(github, context, owner, repo, number, core);
    } catch (error) {
      failures += 1;
